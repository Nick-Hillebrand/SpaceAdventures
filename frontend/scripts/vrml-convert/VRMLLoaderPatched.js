// Vendored from three@0.185.1 (node_modules/three/examples/jsm/loaders/VRMLLoader.js),
// used only by scripts/vrml-convert/convert.mjs — a dev-only, offline VRML97->glTF
// conversion tool (never imported by the app). Patched in `buildNodeMap` below to fix
// an upstream bug: a `children [ ... ]` array that mixes `USE x` references with real
// node definitions gets `field.type` overwritten to `'use'` (the `ctx.use` branch in
// `processField` runs after the `ctx.node` branch and unconditionally overwrites
// `field.type`), so `buildNodeMap`'s `field.type === 'node'` guard skips the field
// entirely and never registers DEFs nested inside the real node siblings. The legacy
// Mars Pathfinder/Sojourner VRML97 assets from mars.nasa.gov hit this: e.g.
// `children [ USE Touch, USE _7, Transform { ... DEF _v2%4 Material {...} ... } ]`.
// Upstream issue: https://github.com/mrdoob/three.js/issues (VRMLLoader mixed
// USE/node MFNode arrays) — not yet reported/fixed as of three@0.185.1.
import {
	BackSide,
	BoxGeometry,
	BufferAttribute,
	...
	parse( data, path ) {
		...
		function buildNodeMap( node ) {
			...
			for ( let i = 0, l = fields.length; i < l; i ++ ) {
				...
				// Patched: recurse based on each value actually being a real node
				// (has `.fields`) rather than trusting `field.type`, which is
				// unreliable for MFNode arrays mixing `USE` refs with real nodes
				...
				if ( field.type === 'node' || field.type === 'use' ) {
					...
					for ( let j = 0, jl = fieldValues.length; j < jl; j ++ ) {
						...
						if ( value && value.fields ) buildNodeMap( value );
		...
		function resolveUSE( identifier ) {
			...
			// Patched: non-visual nodes (e.g. TouchSensor) have no `buildXxxNode`
			// case and `buildNode` returns undefined for them. A `USE` of such a
			// node (VRML allows referencing any DEF, visual or not) previously
			// crashed here reading `.isObject3D` off undefined — just pass the
			// non-build through; `parseFieldChildren`'s `instanceof Object3D`
			// check already skips non-visual values when adding to the scene.
			if ( build === undefined || build === null ) return build;
