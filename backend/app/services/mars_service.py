...
def _photos_path(rover: str) -> str:
    return f"/mars-photos/api/v1/rovers/{rover}/photos"
...
async def fetch_photos(
    ...
    *,
    ...
    existing = await _query_photos(
        session, rover, sol=sol, earth_date=earth_date, camera=camera, page=page
    )
    ...
    photos: list[dict] = payload.get("photos") or [] if isinstance(payload, dict) else []
